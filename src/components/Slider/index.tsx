...
class Slider extends Component<Props, State> {
  static getDerivedStateFromProps(props: Props, state: State) {
    if (!Array.isArray(props.value)) {
      ...
      if (currentPropsValue.length !== state.value.length) {
        const normalizeValues = prepareValuesFromProps(props.value! as number, props.maximumValue!, props.minimumValue!);
  ...
  _fireChangeEvent = (event: SliderEvent) => {
    ...
    if (this.props[event]) {
      // @ts-ignore
      this.props[event].apply(this, this.state.value.map((x: any) => x.__getValue()));
    }
  };
  ...
  _handleMeasure = (name: 'thumbSize' | 'containerSize', event: LayoutChangeEvent) => {
    ...
    if (this._containerSize && this._thumbSize) {
      ...
      if (this.isMultiThumb) {
        ...
        // @ts-ignore
        this.setState({ ...state, valueRailLength, visualRailLength, thumbOffset });
        return;
      }
  ...
  _renderThumbImage = (thumbIndex: number) => {
    ...
    if (Array.isArray(thumbImage)) {
      return (
        <Image source={thumbImage[thumbIndex]} style={{ width: '100%', height: '100%' }} />
      );
    }
  };
